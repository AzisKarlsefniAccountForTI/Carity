...
import { 
  ...
  Activity, AlertTriangle, Eye, EyeOff, Cloud, CloudOff, RefreshCw, Copy, Wallet, FileText
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (syncId) {
      const interval = setInterval(() => pullFromCloud(syncId), 10000);
    ...
    <div className={`min-h-screen bg-[#020617] text-slate-100 transition-all duration-700 ${isEditingVault ? 'overflow-hidden max-h-screen' : 'pb-44'}`}>
      ...
      {!isEditingVault && (
        <div className="fixed top-6 left-6 right-6 z-[60000] flex justify-between items-center">
            ...
            <div className="flex gap-2 items-center">
                {isCloudSynced && (
                   <span className="hidden md:block text-[10px] font-mono text-emerald-500 font-bold uppercase tracking-widest mr-2">
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="space-y-4 text-center md:text-left">
                <div className="flex items-center gap-3 justify-center md:justify-start">
                   ...
                   <span className="text-xs font-black text-indigo-400 uppercase tracking-[0.4em] font-mono">Archive v5.2</span>
                </div>
            ...
            <div className="glass-panel p-10 md:p-14 rounded-[4rem] relative overflow-hidden shadow-2xl border-white/5">
               ...
               <div className="mt-10 h-6 w-full bg-slate-950/50 rounded-full p-1.5 border border-white/5 overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-indigo-600 via-indigo-400 to-emerald-400 rounded-full transition-all duration-1000" style={{ width: `${totals.progress}%` }} />
               </div>
            </div>
          </div>
        </header>
      ...
      <main className={`max-w-4xl mx-auto px-6 relative z-50 transition-all duration-700 ${isEditingVault ? 'hidden' : 'opacity-100'}`}>
        {activeTab === 'home' && (
          <div className="space-y-10 animate-reveal">
            ...
            {/* Manual Entry Form */}
            <div className="glass-panel rounded-[4rem] p-10 md:p-14 border-white/10 space-y-8">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-mono text-slate-500 tracking-widest uppercase font-bold">Manual Protocol Transaction</p>
                  </div>
               </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     {['Azis Khoirul', 'Siska Icha'].map((name) => (
                       <button 
                         ...
                         className={`py-5 rounded-3xl font-black text-xs uppercase tracking-widest transition-all border ${manualSaver === name ? 'bg-white text-slate-950 border-white shadow-2xl' : 'bg-white/5 text-slate-500 border-white/5'}`}
                       >
                         {name.split(' ')[0]}
                       </button>
                     ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div className="space-y-2">
                        ...
                        <input 
                           ...
                           placeholder="Tulis alasan nabung..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                { name: 'Azis Khoirul', label: 'Azis Side', total: totals.azis, progress: totals.azisProgress, color: 'from-blue-600 to-indigo-700', icon: <User size={24} />, accent: 'text-blue-400' },
                { name: 'Siska Icha', label: 'Siska Side', total: totals.siska, progress: totals.siskaProgress, color: 'from-rose-500 to-rose-700', icon: <Zap size={24} />, accent: 'text-rose-400' }
              ].map((saver) => (
                <div key={saver.name} className="glass-panel p-10 rounded-[3.5rem] flex flex-col justify-between">
            ...
            <div className="grid grid-cols-2 gap-6 md:gap-8">
              <div className="glass-panel p-10 rounded-[3.5rem] flex flex-col justify-between">
        ...
        {activeTab === 'menu' && (
          <div className="space-y-10 animate-reveal pb-20">
            ...
            <div className="grid grid-cols-1 gap-6">
               <button onClick={downloadPDF} className="glass-panel p-10 rounded-[3rem] flex items-center justify-between group border-indigo-500/20 bg-indigo-500/5 hover:bg-indigo-500/10 transition-all">
                  <div className="flex items-center gap-8">
                    ...
                    <div className="text-left">
                        ...
                        <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest font-bold">Unduh semua data tabungan & log</span>
                    </div>
                  </div>
                  <Download size={24} className="text-indigo-400 group-hover:scale-110 transition-transform" />
               </button>

               <button onClick={pullFromCloud} className="glass-panel p-10 rounded-[3rem] flex items-center justify-between group">
                  <div className="flex items-center gap-8">
                    ...
                    <div className="text-left">
                        <span className="font-black text-xl block uppercase tracking-tight italic">Sinkron Awan</span>
                        <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest font-bold">Refresh database sekarang</span>
                    </div>
                  </div>
               </button>

               <button onClick={() => confirm('Wipe data?') && setState({transactions:[], emergencyFundAmount:0, vaultLogs:[], isEncrypted: true})} className="glass-panel p-10 rounded-[3rem] flex items-center justify-between group text-rose-500 border-rose-500/20 bg-rose-500/5">
                  <div className="flex items-center gap-8">
                    <div className="w-16 h-16 rounded-[1.5rem] bg-rose-500/10 flex items-center justify-center">
                        <AlertTriangle size={28} />
                    </div>
                    <div className="text-left">
                        <span className="font-black text-xl block uppercase tracking-tight italic">Format Memory</span>
                        <span className="text-[10px] font-mono text-rose-800 uppercase tracking-widest font-bold">Hapus permanen database</span>
                    </div>
                  </div>
               </button>
            </div>
          ...
          <nav className="pointer-events-auto flex items-center justify-around w-full bg-slate-900/95 border border-white/10 shadow-2xl rounded-[2.5rem] p-4 backdrop-blur-3xl">
            {[
              ...
              <button 
                ...
                className={`flex-1 flex flex-col items-center py-4 rounded-3xl transition-all ${activeTab === item.id ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
              >
                <item.icon size={22} />
      ...
      {showSyncModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[90000] flex items-center justify-center p-8 animate-reveal" onClick={() => setShowSyncModal(false)}>
           <div className="glass-panel w-full max-w-md rounded-[4rem] p-12 space-y-10 border-white/10" onClick={e => e.stopPropagation()}>
              <div className="text-center space-y-4">
                 <div className="w-24 h-24 mx-auto rounded-[2.5rem] bg-indigo-600/10 flex items-center justify-center text-indigo-400">
                    <Cloud size={48} className={isSyncing ? 'animate-pulse' : ''} />
                 </div>
                 <h3 className="text-3xl font-black uppercase tracking-tight italic">Sigma Cloud</h3>
                 <p className="text-slate-400 text-[10px] font-black tracking-widest leading-relaxed">SYNC ID: DATABASE KELUARGA</p>
              </div>
              ...
              <div className="space-y-4">
                  <div className="relative">
                    <input 
                       ...
                       className="w-full bg-slate-950 border border-white/10 rounded-2xl py-6 px-8 text-xl font-black font-mono text-white outline-none focus:border-indigo-500"
                    />
                    {syncId && (
                      <button onClick={() => {navigator.clipboard.writeText(syncId); showToast("Copied");}} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500">
                        <Copy size={20} />
                      </button>
                    )}
                  </div>
                  <button onClick={() => setShowSyncModal(false)} className="w-full py-5 rounded-2xl bg-indigo-600 text-white font-black uppercase italic tracking-widest">Connect Node</button>
              </div>
           </div>
        </div>
      )}
          ...
          <div className="flex-none bg-slate-900/90 backdrop-blur-3xl border-b border-white/10 p-10 flex justify-between items-center z-50">
             <div className="flex items-center gap-6">
                ...
                <div>
                   ...
                   <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest font-bold">Authorized Manual Interface</p>
                </div>
             </div>
             <button onClick={() => setIsEditingVault(false)} className="p-5 bg-white/5 rounded-full hover:bg-rose-600 transition-all active:scale-90"><X size={32} /></button>
          </div>
                      ...
                      <div className="space-y-6">
                         <div className="grid grid-cols-1 gap-4">
                            <div className="space-y-2">
                               ...
                               <input type="number" value={manualVaultAmount} onChange={(e) => setManualVaultAmount(e.target.value)} placeholder="0" className="w-full bg-black/80 rounded-[2rem] py-8 px-10 text-3xl font-black border border-white/10 focus:border-amber-500 outline-none text-white font-mono" />
                            </div>
                            <div className="space-y-2">
                               <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Keterangan</label>
                               <input type="text" value={manualVaultReason} onChange={(e) => setManualVaultReason(e.target.value)} placeholder="Alasan operasional..." className="w-full bg-black/80 rounded-2xl py-6 px-10 text-sm border border-white/10 focus:border-amber-500 outline-none text-white font-bold" />
                            </div>
                         </div>
                         <div className="grid grid-cols-2 gap-6">
                            <button onClick={() => handleManualVault('lock')} className="py-6 rounded-[2rem] bg-emerald-600 font-black text-white text-sm shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3 uppercase tracking-widest">
                               <Lock size={22} /> Simpan
                            </button>
                            <button onClick={() => handleManualVault('release')} className="py-6 rounded-[2rem] bg-rose-600 font-black text-white text-sm shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3 uppercase tracking-widest">
                     ...
                     {state.vaultLogs.length === 0 ? (
                        ...
                        state.vaultLogs.map((log) => (
                           <div key={log.id} className="p-8 rounded-[2rem] bg-white/[0.03] border border-white/5 flex items-center justify-between group shadow-xl">
                              <div className="flex items-center gap-8">
                                 <div className={`w-14 h-14 rounded-[1.2rem] flex items-center justify-center shadow-lg ${log.type === 'lock' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>
